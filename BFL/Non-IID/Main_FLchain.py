...
import collections 
#import Models.FederatedLearning as FL
import json
import os
...
def main():
    for i in range(p.Runs):
        ...
        if p.model == 4 and p.FL_TYPE == 1:
            ...
            emnist_train_pruned = tff.simulation.datasets.ClientData.from_clients_and_fn(p.emnist_train.client_ids, FederatedLearning.create_tf_dataset_for_client_fn)
            ...
            local_time_log = []
            round_time_ml_log = []
            tracker = CarbonTracker(epochs = p.NUM_ROUNDS_FL, epochs_before_pred=-1, monitor_epochs=p.NUM_ROUNDS_FL, log_dir=p.path, devices_by_pid = True, update_interval = True)
            round_counter = 0
            ...
            for b_idx, b in enumerate(Consensus.global_chain):
                ...
                if len(b.transactions) > 0 and round_counter < p.NUM_ROUNDS_FL: # added stopping condition according to the number of FL rounds
                    ...
                    state, train_metrics, val_metrics, local_time = \
                        FL.execute_fl_round(emnist_train_pruned, iterative_process, state, fed_evaluation, fed_evaluation_data, b.transactions)
            ...
            plt.savefig(p.path+"dataset_distribution.jpg")
