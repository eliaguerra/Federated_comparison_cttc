...
class FederatedLearning():
    ...
    def initialize_fl_model(self):
        ...
        for id in p.emnist_train.client_ids:
            classes_set = np.random.choice(range(0,10), p.NUM_CLASSES_FOR_USER, replace = False)
            client_dataset = p.emnist_train.create_tf_dataset_for_client(id)
    ...
    # Execute an FL round
    def execute_fl_round(self, emnist_train, iterative_process, state, fed_evaluation, federated_evaluation_data, transactions):
        ...
        federated_train_data = self.make_federated_data(emnist_train, participating_clients_ids)
