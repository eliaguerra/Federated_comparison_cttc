...
class BlockCommit(BaseBlockCommit):
    ...
    def receive_block(event):
        ...
        if blockPrev == lastBlockId:
            ...
            if node.hashPower > 0: BlockCommit.generate_next_block(node, currentTime)  # Start mining or working on the next block
    ...
    def propagate_block(block):
        for recipient in p.NODES:
            if recipient.id != block.miner:
                blockDelay = random.expovariate(
                    1 / p.Bdelay)  # Network.block_prop_delay()  # draw block propagation delay from a distribution !! or you can assign 0 to ignore block propagation delay
